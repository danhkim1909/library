			...
			<div class="row">
					...
					<div class="shop_sidebar">
						...
						<div class="widget shop_category_widget">
							...
							<ul class="list-unstyled shop_category_list">
								<li><a th:href="@{/sach(tuKhoa=${tuKhoa}, maTacGia=${maTacGia}, maTheLoai=0, sapXepTheo=${sapXepTheo})}"
						...
						<div class="widget author_widget">
							...
							<ul class="list-unstyled shop_category_list">
								<li><a th:href="@{/sach(tuKhoa=${tuKhoa}, maTacGia=0, maTheLoai=${maTheLoai}, sapXepTheo=${sapXepTheo})}"
				...
				<div class="col-lg-9">
					...
					<div class="shop_top d-flex align-items-center justify-content-between">
						<div class="shop_menu_left" th:text="${'Tìm được ' + truyVan.totalElements + ' sách'}">500
							sách</div>
						<div class="shop_menu_right d-flex align-items-center justify-content-end">
							Sắp xếp theo:
							<div class="woocommerce-ordering" method="get">
								<select name="orderby" class="orderby selectpickers" id="selectOrder">
									<option value="ngayNhap_desc" th:selected="${sapXepTheo == 'ngayNhap_desc'}">Mới
										nhất</option>
									<option value="ngayNhap_asc" th:selected="${sapXepTheo == 'ngayNhap_asc'}">Cũ nhất
									</option>
									<option value="tenSach_asc" th:selected="${sapXepTheo == 'tenSach_asc'}">Tên từ A
										đến Z</option>
									<option value="tenSach_desc" th:selected="${sapXepTheo == 'tenSach_desc'}">Tên từ Z
										đến A</option>
									<option value="soLanMuon_asc" th:selected="${sapXepTheo == 'soLanMuon_asc'}">Số lần
										mượn tăng dần</option>
									<option value="soLanMuon_desc" th:selected="${sapXepTheo == 'soLanMuon_desc'}">Số
										lần mượn giảm dần</option>
								</select>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-xl-3 col-lg-4 col-sm-6 projects_item" th:each="sach : ${truyVan.content}">
								...
								<div class="bj_new_pr_content">
									...
									<div class="bj_pr_meta d-flex">
										<div class="book_price" th:text="${sach.moTa}"
											style="font-weight: normal; color: #6c757d;">Mô tả</div>
									</div>
								</div>
							</div>
						</div>
						...
						<nav aria-label="...">
							<ul class="pagination pagi-content">
								<li th:if="${!truyVan.isFirst()}" class="page-item">
									<a class="page-link"
										th:href="@{/sach(tuKhoa=${tuKhoa}, maTacGia=${maTacGia}, maTheLoai=${maTheLoai}, sapXepTheo=${sapXepTheo}, trang=${truyVan.number})}"><i
											class="ti-angle-left"></i>Trước</a>
								</li>
								<li th:each="i : ${#numbers.sequence(1, truyVan.totalPages)}" class="page-item"
									aria-current="page" th:classappend="${truyVan.number + 1 == i ? 'active' : ''}">
								...
								<li th:if="${!truyVan.isLast()}" class="page-item">
									<a class="page-link"
										th:href="@{/sach(tuKhoa=${tuKhoa}, maTacGia=${maTacGia}, maTheLoai=${maTheLoai}, sapXepTheo=${sapXepTheo}, trang=${truyVan.number + 2})}">Sau<i
